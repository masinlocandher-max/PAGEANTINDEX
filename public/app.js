...
if (isAdminHost) {
  document.title = "Admin Dashboard | PageantIndex";
...
function brandLockup(inverse = false) {
  return `<span class="brand-lockup${inverse ? " inverse" : ""}"><img src="/public/images/pageant-icon.png" alt=""><span class="brand-type"><strong>Pageant<span>Index</span></strong><small>The Global Network for Pageantry</small></span></span>`;
}
function announcementBar() {
  const message = `<span>PageantIndex is building the global professional record for pageantry</span><i aria-hidden="true"></i><span>Free founding profiles are now open</span><i aria-hidden="true"></i><a href="/sign-up/">Create your profile</a>`;
...
function header() {
  ...
  return `<header class="site-header global-header"><div class="container nav-wrap"><a class="brand" href="/" aria-label="PageantIndex home">${brandLockup()}</a><nav class="desktop-nav" aria-label="Primary navigation">${nav.map(([l, u]) => `<a class="${path === u ? "active" : ""}" href="${u}">${l}</a>`).join("")}</nav><div class="nav-actions"><a class="signin" href="/sign-in/">Sign in</a><a class="btn btn-primary" href="/sign-up/">Create profile</a><button class="menu-toggle" aria-label="Open menu" aria-expanded="false">${svgIcon("menu")}</button></div></div><nav class="mobile-nav" aria-label="Mobile navigation">${nav.map(([l, u]) => `<a href="${u}">${l}</a>`).join("")}<a href="/sign-in/">Sign in</a><a class="mobile-nav-cta" href="/sign-up/">Create your free profile</a></nav></header>`;
}
function footer() {
  return `<footer class="site-footer global-footer"><div class="container"><div class="footer-grid"><div class="footer-brand">${brandLockup(true)}<p>The global professional identity and relationship infrastructure for pageantry.</p><a class="footer-domain" href="https://www.pageantindex.com/">www.pageantindex.com</a></div><div class="footer-col"><h4>Discover</h4><a href="/directory/">Explore the index</a><a href="/candidates/">Candidates</a><a href="/organization/">Organizations</a><a href="/articles/">Industry knowledge</a></div><div class="footer-col"><h4>Build your record</h4><a href="/sign-up/">Create a free profile</a><a href="/claim-profile/">Claim a profile</a><a href="/dashboard/">Profile dashboard</a><a href="/verification/">Trust and verification</a></div><div class="footer-col"><h4>PageantIndex</h4><a href="/experience/">How it works</a><a href="/about/">About the platform</a><a href="/ranking-methodology/">Standards</a><a href="/about/#privacy">Privacy</a></div></div><div class="footer-bottom"><span>© 2026 PageantIndex. All rights reserved.</span><span>One global platform. One connected industry record.</span></div></div></footer>`;
}

function homePage() {
  const relationship = [["building-2","Organization","Owns the brand and appoints its team"],["crown","Edition","The specific year, city, and title"],["file-user","Candidate","Builds a verified pageant history"],["sparkles","Professional","Connects expertise to a confirmed role"]];
  return `<main class="page-shell global-home">
  <section class="global-hero"><div class="global-hero-media" role="img" aria-label="Pageant candidates and professionals working together backstage"></div><div class="global-hero-shade"></div><div class="container global-hero-inner"><div class="global-hero-copy"><h1>The pageant world,<br><em>connected.</em></h1><p>Create your verified professional identity. Document pageant history. Connect the people and organizations behind every title.</p><div class="global-hero-actions"><a class="btn btn-primary" href="/sign-up/">Create your free profile</a><a class="btn btn-outline-light" href="/directory/">Explore the index</a></div></div><div class="connection-trace" aria-hidden="true"><span></span><span></span><span></span></div></div></section>
  <section class="global-search"><div class="container global-search-grid"><div><h2>Explore the global index</h2><p>Search organizations, editions, candidates, professionals, and confirmed roles across the pageant ecosystem.</p></div><form class="index-search" data-main-search><select aria-label="Search type"><option>All records</option><option>Organizations</option><option>Editions</option><option>Candidates</option><option>Professionals</option></select><label><span class="sr-only">Search PageantIndex</span><input name="q" placeholder="Search names, titles, countries, or roles"></label><button class="btn btn-primary">Search</button></form></div></section>
  <section class="relationship-section"><div class="container relationship-layout"><div class="relationship-intro"><h2>One industry.<br>One connected record.</h2><p>Every title is the result of people, roles, and relationships working together. PageantIndex turns that history into a structured, searchable professional record.</p><a href="/experience/">See how it connects ${svgIcon("arrow","inline-icon")}</a></div><div class="relationship-flow">${relationship.map(([icon,title,copy],i)=>`<div class="relationship-node"><span>${assetIcon(icon)}</span><strong>${title}</strong><small>${copy}</small>${i<relationship.length-1?'<i aria-hidden="true">→</i>':''}</div>`).join("")}<div class="credit-rail">Pageant + Edition + Candidate + Professional + Role</div></div></div></section>
  <section class="identity-paths"><div class="container"><div class="paths-heading"><h2>Three ways to join the index.</h2><p>Choose the identity that represents your role in pageantry. Every profile begins free.</p></div><div class="path-list">${[["file-user","I am a Professional","Showcase your expertise, portfolio, locations, services, and confirmed pageant credits.","Create professional profile","/sign-up/?role=supplier"],["crown","I am a Candidate","Build a public identity, document titles and placements, and confirm your personal team.","Create candidate profile","/sign-up/?role=candidate"],["building-2","I represent an Organization","Claim your brand, manage editions and candidates, and confirm official suppliers.","Create organization profile","/sign-up/?role=organization"]].map(([icon,title,copy,cta,url])=>`<article class="path-item"><span>${assetIcon(icon)}</span><div><h3>${title}</h3><p>${copy}</p><a href="${url}">${cta} ${svgIcon("arrow","inline-icon")}</a></div></article>`).join("")}</div></div></section>
  <section class="profile-showcase"><div class="container profile-showcase-grid"><div class="profile-story"><h2>A verified identity.<br>A public record.</h2><p>Your PageantIndex profile brings identity, work, roles, locations, portfolio, and official links into one professional page the industry can understand and trust.</p><a href="/sign-up/">Build your profile ${svgIcon("arrow","inline-icon")}</a></div><div class="profile-product"><header><div class="profile-avatar">PI</div><div><h3>Professional profile <b title="Identity verified">✓</b></h3><p>Creative professional · Available worldwide</p></div><button class="btn btn-primary btn-small">Connect</button></header><div class="profile-product-body"><div><small>CONFIRMED PAGEANT CREDITS</small><ul><li><span>International pageant</span><b>Creative Director</b><em>2026</em></li><li><span>National pageant</span><b>Official Photographer</b><em>2025</em></li><li><span>Regional pageant</span><b>Wardrobe Consultant</b><em>2024</em></li></ul></div><div><small>PORTFOLIO & OFFICIAL LINKS</small><div class="portfolio-preview">${["camera","sparkles","crown","building-2"].map(icon=>`<span>${assetIcon(icon)}</span>`).join("")}</div><p>Website · Portfolio · Social profiles</p></div></div></div></div></section>
  <section class="trust-section"><div class="container trust-layout"><div><h2>Built on trust.<br>Verified by design.</h2><p>Three distinct trust layers show exactly what PageantIndex verified and who confirmed each relationship.</p></div><div class="trust-list">${[["shield-check","Identity Verified","PageantIndex confirms the person or organization using private identity evidence."],["badge-check","Candidate Confirmed","A candidate confirms that a professional worked directly with her in the stated role."],["building-2","Organization Confirmed","An organization confirms an official pageant-level role for a specific edition."]].map(([icon,title,copy])=>`<article><span>${assetIcon(icon)}</span><h3>${title}</h3><p>${copy}</p></article>`).join("")}</div></div></section>
  <section class="global-cta"><div class="sarimanok-sweep" aria-hidden="true"></div><div class="container"><h2>The global network<br>for pageantry.</h2><p>Create your free profile and become part of the connected industry record.</p><a class="btn btn-primary" href="/sign-up/">Create your free profile</a></div></section>
  </main>`;
}
...
function initDashboard() {
  ...
  const renderProfileState = () => {
    ...
    document.getElementById("live-location").textContent = fields.location.value || "Location · Available worldwide";
  ...
  document.getElementById("open-full-preview")?.addEventListener("click", () =>
    openModal("Public profile preview", `<div class="preview-dialog"><h3>${escapeHtml(fields.business.value || "Your business name")}</h3><p>${escapeHtml(fields.category.value)} · ${escapeHtml(fields.location.value || "Location · Available worldwide")}</p><h4>About</h4><p>${escapeHtml(fields.about.value || "Your professional introduction will appear here.")}</p><h4>Services</h4><p>${escapeHtml(fields.services.value || "Your services will appear here.")}</p></div>`));
